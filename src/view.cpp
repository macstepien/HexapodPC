...
void View::update(char key, Robot& rob)
{
    ...
    drawAxis(rob.getPosition(), rob.getAngles());
    drawRobot(rob);
...
void View::drawAxis(cv::Point3f pt, cv::Point3f angles)
{
    drawPoint(pt);
    ...
    drawLine(pt, p1, Scalar(255,0,0), 2);//x niebieski
    drawLine(pt, p2, Scalar(0,255,0), 2);//y zielony
    drawLine(pt, p3, Scalar(0,0,255), 2);//z czerwony
}
